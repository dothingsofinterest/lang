...
import { LoadingOutlined, RedoOutlined, UploadOutlined, FastBackwardOutlined, PauseCircleOutlined, FastForwardOutlined, PlayCircleOutlined } from "@ant-design/icons";
...
const Follow = () => {
    ...
    const handleInput = (value: string) => {
        ...
        if (inputMode) {
            ...
            if (subtitles[subtitlesCurIndex].text === value) {
                setInputValue("");
            ...
            if (words[wordsCurIndex].split(" ")[0] === value.replaceAll(" ", "/")) {
                setInputValue("");
    ...
    const playAudio = async (index: number) => {
        if (words && words.length) {
            const en = words[index].split(" ");
            const content = dictationWordsMode ? en[0].replaceAll("/", ", ") : en[1].split(".")[1];
                                                ...
                                                {value.children.map((v, k) => {
                                                    return (
                                                        <React.Fragment key={v.key}>
                                                            ...
                                                            <span className="sub">{v.text}</span>
                                                        </React.Fragment>
                                                    );
                                                })}
                                            </p>
