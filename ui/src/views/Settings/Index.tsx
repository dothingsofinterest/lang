...
import { videoImport, videoInit, importData, exportData, statisticsCountVocab, statisticsSearch, concatAudio, speechBatchTranscode } from "../../api/requestAuth";
    ...
    const handlersExportAudio = async () => {
        if (data.videoHash && data.videoURL) {
            try {
                ...
                const writable = await handle.createWritable();
                const resBlob = await concatAudio({ hash: data.videoHash });
